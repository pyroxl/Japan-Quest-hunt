...
The app works offline and therefore uses a network-first service-worker cache. This update uses cache `japan-quest-v25`, checks for updates on every launch, and falls back to saved files when offline. If an already-open tab still shows the old route:
