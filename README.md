...
The itinerary data reflects the current Tokyo–Hakone–Tokyo sequence: Mai and Brian take small bags to Gora Nov 8–11, return for one Tokyo night (Nov 11–12), then spend Nov 12 moving around the city with bags stored before the 01:00 Haneda flight.
...
The app works offline and therefore uses a network-first service-worker cache. This update uses cache `japan-quest-v165`, checks for updates on every launch, and falls back to saved files when offline. If an already-open tab still shows the old route or styling:
